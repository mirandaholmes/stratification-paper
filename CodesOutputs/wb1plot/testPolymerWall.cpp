...
int main(int argc,char *argv[])
{
     ...
     /*  ------  Set up parameters  ------ */
    // Sampling parameters
    int npts = 2e4;   // how many points to generate
    int dsave = 100;         // how often to save points
    
    int ifwritepts = 1;  // set to 1 to write points, labels to file
    
    // Stratification parameters
    int n = 20;        // number of spheres
    ...
    // filename
    string filehead("PolymerWall_kbend_n20_k1");
    ...
    // Proposal parameters
    int seed = -1431903999;     // = 0 for random seed (based on clock time)
    double sig = 0.15;  
    double sigbdy = 0.3;   //Point::inf
    double sigtan = 0.2;
    double lamlose = 0.4;
    double lamgain = 2. * sigbdy*lamlose;  // smaller for higher kappa
