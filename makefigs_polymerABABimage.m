...
ifsave = 1;
savefile = 'polymerABAB_image.epsc';
...
del = 0;
axC = axes(gcf,'Position',[0+del 0+del 1-2*del 1-2*del]);  % figure goes from 0 to 1, takes full window
...
if(iftext)
    for jn=1:n
        text(X(1,jn)-tshift,X(2,jn)-tshift,X(3,jn)+tshift,tt{jn},'FontSize',tfs,...
